...
- `assets/samples/battle/`: 検証用の対戦画面サンプル画像置き場です。
- `assets/debug/`: crop 画像、前処理後画像、observation JSON、overlay response、validation report などの debug 出力先です。単発実行は `single-run/`、一括検証は `validation/` に分けます。生成物は git 管理対象外です。
...
PYTHONPATH=vision-py/src python3 -m vision.main \
  --image assets/samples/battle/battle_sample.jpeg \
  --ocr-names \
  --request-overlay
```
...
省略時の保存先は `assets/debug/single-run/observation.json` です。
...
省略時の保存先は `assets/debug/single-run/overlay_response.json` です。
...
`assets/samples/battle/` 配下の `.png` / `.jpg` / `.jpeg` をまとめて処理します。
...
- `assets/debug/validation/<image_stem>_<ext>/`: 画像ごとの crop、前処理後画像、gender crop、observation JSON

`validation_report.json` には `tuning_parameters` として、OCR 前処理、gender 判定、基準解像度の現在値も出力されます。調整時は `vision-py/src/vision/tuning.py` の値を変更してから再実行し、`success` / `partial` / `failed` と OCR raw text、gender reason の変化を比較します。
...
### `assets/samples/battle/`

vision-py の入力サンプル画像置き場です。validation はこのディレクトリの `.png` / `.jpg` / `.jpeg` を走査します。再現用 sample と実行結果を混ぜないため、debug 出力は置きません。
...
省略時の出力先は `assets/debug/` です。単発実行の成果物は `assets/debug/single-run/`、validation の画像別成果物は `assets/debug/validation/` に保存されます。

- `single-run/opponent_status_panel_raw.png` / `single-run/player_status_panel_raw.png`: status panel crop
- `single-run/opponent_name_raw.png` / `single-run/player_name_raw.png`: OCR 対象の raw crop
- `single-run/opponent_name_preprocessed.png` / `single-run/player_name_preprocessed.png`: OCR に渡す代表前処理画像
- `single-run/opponent_gender_raw.png` / `single-run/player_gender_raw.png`: gender 判定用 crop
- `single-run/observation.json`: vision-py が生成した observation DTO
- `single-run/overlay_response.json`: Go API から返った overlay DTO、または通信失敗時の error payload
- `validation_report.json`: 複数サンプル検証のサマリーと画像ごとの詳細
- `validation/<image_stem>_<ext>/`: 画像ごとの crop、前処理、gender、observation の保存先

調整対象の主要パラメータは `vision-py/src/vision/tuning.py` にまとめています。現在は OCR 前処理の `resize_factor` / `brightness_threshold` / `trim_padding` / `binary_threshold`、gender 判定の score threshold / margin、region の基準解像度と基準座標を管理します。
...
python3 -m json.tool shared/master-data/pokemon.json >/dev/null
python3 -m json.tool shared/player-config/player_speed.json >/dev/null
